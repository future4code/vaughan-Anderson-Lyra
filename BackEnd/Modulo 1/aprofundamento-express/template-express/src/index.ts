...
app.get('/afazeres/:status', (req, res) => {
   if(req.params.status === 'true' || req.params.status === 'false'){
       ...
       if (req.params.status === 'true'){
         ...
         let listaDeAfazeres = afazeres.filter((afazer) => {
             return afazer.completed === status})
   
            res.send(listaDeAfazeres)
        }else{
    res.send("Não esta funcionando") 
}})

//5)
app.post('/addAfazeres', (req, res) => { 
    
    let novoAfazer: Exemplos = req.body
...
app.delete('/afazeres/:id', (req, res) => {
    ...
    let afazer = afazeres.find((afazer) => {
        return afazer.id === id
    }) 
    if(afazer){
...
//9)
// https://documenter.getpostman.com/view/19296981/UVypxwRf